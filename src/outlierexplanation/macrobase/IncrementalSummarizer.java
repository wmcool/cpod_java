...
public class IncrementalSummarizer implements IncrementalOperator<Explanation> {
    ...
    private Deque<HashMap<Set<Integer>, Double>> inlierItemsetPaneCounts; // 每个inlier attr combination在每个pane中的数量（先进后出）
    private Deque<HashMap<Set<Integer>, Double>> outlierItemsetPaneCounts; // 每个outlier attr combination在每个pane中的数量（先进后出）
    private HashMap<Set<Integer>, Double> inlierItemsetPaneCount = new HashMap<>(); // 每个inlier attr combination在当前pane中的数量
    private HashMap<Set<Integer>, Double> outlierItemsetPaneCount = new HashMap<>(); // 每个outlier attr combination在当前pane中的数量
    private HashMap<Set<Integer>, Double> inlierItemsetWindowCount = new HashMap<>(); // 每个inlier attr combination在当前window中的数量
    private HashMap<Set<Integer>, Double> outlierItemsetWindowCount = new HashMap<>(); // 每个outlier attr combination在当前window中的数量
    private Deque<Integer> inlierPaneCounts; // 每个pane中inlier的数量（先进后出）
    private Deque<Integer> outlierPaneCounts; // 每个pane中outlier的数量（先进后出）
    private HashMap<Set<Integer>, Integer> trackingMap = new HashMap<>(); // 第一次遇到attr combination的pane num

    // Temp Intermediate Values
    private List<Integer> inlierCountCumSum; // 前缀数组
