...
public class DataFrame {
    ...
    public ArrayList<double[]> getDoubleCols(List<Integer> columns) {
        ...
        for (int c : columns) {
            if (schema.getColumnType(c) != Schema.ColType.DOUBLE) {
    ...
    public ArrayList<String[]> getStringCols(List<Integer> columns) {
        ...
        for (int c : columns) {
            if (schema.getColumnType(c) != Schema.ColType.STRING) {
                throw new UnsupportedOperationException(
    ...
    public DataFrame project(List<String> projectionCols) {
        ...
        for (String col : projectionCols) {
            ...
            final Schema.ColType type = schema.getColumnTypeByName(col);
            if (type == Schema.ColType.DOUBLE) {
                other.addColumn(col, getDoubleColumnByName(col));
            } else if (type == Schema.ColType.STRING) {
    ...
    public DataFrame project(String projectionCol) {
        ...
        final Schema.ColType type = schema.getColumnTypeByName(projectionCol);
        if (type == Schema.ColType.DOUBLE) {
    ...
    public DataFrame orderBy(final String sortCol, final boolean sortAsc) {
        ...
        final Schema.ColType sortColType = this.schema.getColumnTypeByName(sortCol);
        final int numColumns = this.schema.getNumColumns();

        if (sortColType == Schema.ColType.DOUBLE) {
    ...
    private void sortColumns(final DataFrame sortedDf, final int numColumns,
        ...
        for (int c = 0; c < numColumns; ++c) {
            if (this.schema.getColumnType(c) == Schema.ColType.DOUBLE) {
